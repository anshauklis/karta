...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/node-25-339933?logo=node.js&logoColor=white" alt="Node" />
</p>
...
- **SSO** — OIDC, SAML (via BoxyHQ), LDAP authentication
- **Audit Log** — track every user action with filterable event log
- **RBAC & Teams** — role hierarchy (viewer -> editor -> admin -> owner), team-scoped resources
- **White-Label** — custom logo, favicon, app name, colors, CSS with live preview
- **Multi-Tenant** — schema-per-tenant isolation for SaaS deployments
- **Stripe Billing** — checkout, subscription management, customer portal
- **Managed Cloud** — wildcard subdomain routing, self-service tenant provisioning
- **Semantic Layer** — reusable models, measures, and dimensions — query without SQL
...
Open [http://localhost:8090](http://localhost:8090) and create your admin account on the setup screen.
...
| **frontend** | Node 25, Next.js 16  | Web UI               | 768 MB |
...
| `PORT` | No | `8090` | External HTTP port |
| `NEXTAUTH_URL` | No | `http://localhost:8090` | Public URL for auth callbacks |
| `DOMAIN` | No | — | Domain name for SSL mode |
| `REDIS_URL` | No | `redis://redis:6379/0` | Redis connection URL |
| `REDIS_PASSWORD` | Yes | Auto-generated | Redis authentication password |
...
**Backend:** Python 3.13, FastAPI, SQLAlchemy (raw SQL), psycopg2, PyJWT, bcrypt, pandas, NumPy, SciPy, Plotly, cryptography, APScheduler, Redis, httpx, ldap3, stripe
...
- **[User Guide](docs/user-guide/)** — connections, SQL Lab, dashboards, charts, filters
- **[Administration](docs/admin/)** — users, RLS, alerts, teams, SSO, audit
- **[Deployment](docs/deployment/)** — quickstart, SSL, configuration, architecture, scaling
- **[API Reference](docs/api/README.md)** — full REST API documentation
- **[Semantic Layer](docs/semantic-layer.md)** — models, measures, dimensions
