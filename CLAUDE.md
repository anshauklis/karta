...
Karta is a self-hosted BI platform (like Apache Superset). **Frontend** — Next.js 16 app in `frontend/`. **Backend** — FastAPI app in `api/`. Both run in Docker via `docker-compose.yml`.
...
nginx splits `/api/auth/(session|csrf|callback|signin|signout|providers|error)` to Next.js (NextAuth), all other `/api/*` to FastAPI. Config: `nginx.conf`.
...
- `(dashboard)/` — all protected routes wrapped in `AppShell` (header + auth guard)
  ...
  - `shared/[token]` — public shared dashboard (no auth)

### Chart Editor

The chart editor (`dashboard/[slug]/chart/[id]/`) is the most complex page. Key files:
- `hooks/use-chart-editor.ts` — main orchestration hook (state, save, undo/redo, draft auto-save)
- `components/chart-header.tsx` — title, connection/dataset selectors, save/run buttons
...
FastAPI with raw SQL (SQLAlchemy `text()` queries, no ORM). Key modules:
...
- `charts/router.py` — CRUD + execute/preview with data pipeline (time grain, filters, metrics, calculated columns)
...
| api | ./api (Python 3.11) | 8000 | 1GB |
| frontend | ./frontend (Node 20) | 3000 | 768MB |
...
Root `.env` (used by docker-compose): `POSTGRES_PASSWORD`, `JWT_SECRET`, `CONNECTION_SECRET`, `PORT`, `NEXTAUTH_URL`.
